...
while True:
    ...
    img, conts = cm.getContours(img, minArea=2000, showCanny=True, cannyResize=True, filter=4,
                                 draw=True)  # Funktionens færdigprocesserede billede returneres til "img". - finalContours returneres til "conts". - minArea sættes til 5000 for kun at detektere store objekter på billedet - filter sættes til 4 for kun at få objekter med over 4 hjørner.

    if len(conts) != 0:
        biggest = conts[0][
            2]  # Plads [0] peger på den største kontur i listen "approx". - Plads[2] peger på listen "approx" i finalContours
    ...
    for x in range(len(green_contours)):
        if green_contours[x].size > 300:
            ...
            cv.putText(img2, str(green_rect[-1]), (10, 20), font, fontScale, (0, 255, 0),
                       lineType)  # print rotation of box

            greenBox = cv.boxPoints(green_rect)
            greenBox = np.int0(greenBox)

            cv.drawContours(img2, [greenBox], 0, (0, 255, 0), 1)

            # cv.putText(img2, str(greenBox[0]), (greenBox[0][0], greenBox[0][1]), green_font, green_fontScale,
            #            green_fontColor, green_lineType)
            # cv.putText(img2, str(greenBox[1]), (greenBox[1][0], greenBox[1][1]), green_font, green_fontScale,
            #            green_fontColor, green_lineType)
            # cv.putText(img2, str(greenBox[2]), (greenBox[2][0], greenBox[2][1]), green_font, green_fontScale,
            #            green_fontColor, green_lineType)
            # cv.putText(img2, str(greenBox[3]), (greenBox[3][0], greenBox[3][1]), green_font, green_fontScale,
            #            green_fontColor, green_lineType)
            ...
            cX = int(M["m10"] / M["m00"])
            cY = int(M["m01"] / M["m00"])
            cv.circle(img2, (cX, cY), 5, green_fontColor, -1)
            cv.putText(img2, str([cX,cY]), (cX - 25, cY - 25), green_font, green_fontScale,
                       green_fontColor, green_lineType)
    ...
    for j in range(len(blue_contours)):
        if blue_contours[j].size > 300:
            ...
            cv.putText(img2, str(blue_rect[-1]), (10, 40), font, fontScale, (255, 0, 0),
                       lineType)

            blueBox = cv.boxPoints(blue_rect)
            blueBox = np.int0(blueBox)

            cv.drawContours(img2, [blueBox], 0, (255, 0, 0), 1)

            cv.putText(img2, str(blueBox[0]), (blueBox[0][0], blueBox[0][1]), blue_font, blue_fontScale, blue_fontColor,
                       blue_lineType)
            cv.putText(img2, str(blueBox[1]), (blueBox[1][0], blueBox[1][1]), blue_font, blue_fontScale, blue_fontColor,
                       blue_lineType)
            cv.putText(img2, str(blueBox[2]), (blueBox[2][0], blueBox[2][1]), blue_font, blue_fontScale, blue_fontColor,
                       blue_lineType)
            cv.putText(img2, str(blueBox[3]), (blueBox[3][0], blueBox[3][1]), blue_font, blue_fontScale, blue_fontColor,
                       blue_lineType)
            ...
            cX = int(M["m10"] / M["m00"])
            cY = int(M["m01"] / M["m00"])
            cv.circle(img2, (cX, cY), 5, blue_fontColor, -1)
            cv.putText(img2, str([cX,cY]), (cX - 25, cY - 25), blue_font, blue_fontScale, blue_fontColor,
                       blue_lineType)
    ...
    for i in range(len(red_contours)):
        if red_contours[i].size > 300:
            ...
            cv.putText(img2, str(red_rect[-1]), (10, 60), font, fontScale, (0, 0, 255),
                       lineType)  # print rotation of box

            redBox = cv.boxPoints(red_rect)
            redBox = np.int0(redBox)

            cv.drawContours(img2, [redBox], 0, (0, 0, 255), 1)

            # cv.putText(img2, str(redBox[0]), (redBox[0][0], redBox[0][1]), red_font, red_fontScale, red_fontColor,
            #            red_lineType)
            # cv.putText(img2, str(redBox[1]), (redBox[1][0], redBox[1][1]), red_font, red_fontScale, red_fontColor,
            #            red_lineType)
            # cv.putText(img2, str(redBox[2]), (redBox[2][0], redBox[2][1]), red_font, red_fontScale, red_fontColor,
            #            red_lineType)
            # cv.putText(img2, str(redBox[3]), (redBox[3][0], redBox[3][1]), red_font, red_fontScale, red_fontColor,
            #            red_lineType)
            ...
            cX = int(M["m10"] / M["m00"])
            cY = int(M["m01"] / M["m00"])
            cv.circle(img2, (cX, cY), 5, red_fontColor, -1)
            cv.putText(img2, str([cX,cY]), (cX - 25, cY - 25), red_font, red_fontScale, red_fontColor,
                       red_lineType)
    cv.imshow("vis kasse", img2)
